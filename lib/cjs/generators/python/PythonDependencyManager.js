...
class PythonDependencyManager extends AbstractDependencyManager_1.AbstractDependencyManager {
     ...
     */
    renderDependency(model, packageName) {
        return `from ${packageName}.${helpers_1.FormatHelpers.snakeCase(model.name)} import ${model.name}`;
    }
    /**
     ...
     */
    mergeIndividualDependencies(individualDependencies) {
        const importMap = {};
        ...
        for (const [from, toImport] of Object.entries(importMap)) {
            ...
            dependenciesToRender.push(`from ${from} import ${uniqueToImport.join(', ')}`);
        }
        return dependenciesToRender;
    }
    /**
     ...
     */
    moveFutureDependency(individualDependencies) {
        const futureDependencyIndex = individualDependencies.findIndex((element) => element.includes('__future__'));
        if (futureDependencyIndex !== -1) {
            individualDependencies.unshift(individualDependencies.splice(futureDependencyIndex, 1)[0]);
        }
